...
export function StickyCta({ href = "#acces", label }: { href?: string; label: string }) {
  ...
  return (
    <div
      className={`fixed inset-x-0 bottom-0 z-40 border-t-2 border-orange-dark bg-white p-2.5 shadow-[0_-3px_12px_rgba(0,0,0,0.18)] transition-transform lg:hidden ${
    ...
    >
      <a
        ...
        className="flex min-h-[54px] w-full items-center justify-center rounded border-b-4 border-orange-dark bg-orange px-4 text-center text-[1.05rem] font-bold leading-tight text-white no-underline"
      >
        {label}
      </a>
    </div>
