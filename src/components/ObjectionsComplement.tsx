import { FAQ } from "./ui";
export function ObjectionsComplement({av=false,dossier=false,testament=false}:{av?:boolean;dossier?:boolean;testament?:boolean}){
 const items=[
 ...
 if(testament){
  items[0]={q:"« Est-ce que ce dossier rédige mon testament ? »",a:"Non. Il clarifie vos volontés, repère les informations manquantes et prépare les questions à remettre au notaire. Aucun modèle n’est présenté comme un testament à recopier ou à signer. La validité, la rédaction et les effets doivent être vérifiés par le professionnel."};
  items.splice(1,0,{q:"« Pourquoi payer si je peux écrire mes volontés sur une feuille ? »",a:"Parce que le risque n’est pas seulement d’oublier d’écrire. Il est de confondre souhait moral, clause d’assurance-vie, donation et disposition testamentaire, ou d’ignorer une part protégée. Le dossier vous aide à détecter ces angles morts avant la formalisation."});
 }
