...
export function VideoEmbed({ id, title, minutes }: { id?: string; title: string; minutes?: number }) {
  if (!id) {
    return (
      <div
        ...
        className="relative flex aspect-video w-full flex-col items-center justify-center rounded border border-grey-line bg-[#1b2633] text-white"
      >
        ...
        <span className="absolute bottom-2 right-2 rounded bg-white/15 px-2 py-0.5 text-[0.7rem]">
          vidéo à intégrer
        </span>
      </div>
    );
  ...
  return (
    <div className="aspect-video w-full overflow-hidden rounded border border-grey-line bg-black">
      <iframe
        src={src}
        title={title}
