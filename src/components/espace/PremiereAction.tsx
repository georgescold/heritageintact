...
export function PremiereAction({ etat }: { etat: EtatEspace }) {
  // Guide seul : pas de bloc. Il répétait le téléchargement du guide juste sous
  // la carte « Votre achat » qui propose déjà ce même bouton (retour de Loys, 13/09).
  if (!( ["upsell1", "bump", "upsell2", "backend4"] as const).some(sku => etat.possede.has(sku))) return null;
