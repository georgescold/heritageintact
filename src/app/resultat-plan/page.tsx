...
export default async function Page({searchParams}:{searchParams:Promise<{o?:string}>}) {
  ...
  const suite=`/bienvenue?o=${encodeURIComponent(order.id)}&retour=1`;
  return <><Header minimal/><main className="wrap flex-1 py-8">
    <p className="border-l-4 border-green bg-green-bg p-4 font-bold">Paiement accepté : votre plan adapté à votre situation est déverrouillé.</p>
    <h1 className="my-5 text-[2rem]">Voici votre résultat et l’ordre de préparation correspondant</h1>
    <SimulationPlan verrouille={false} jeton={acces?.jeton}/>
