...
import { AvantApresHistorique, EcheanceHistorique, DernierMotHistorique } from "@/components/marketing/SectionsHistoriques";
...
function AccesMethode({ montant, sombre = false }: { montant: number; sombre?: boolean }) {
  return <div data-mesure="clic_commande">
    <a href="/commande" className="flex min-h-[58px] w-full items-center justify-center border-b-4 border-orange-dark bg-orange px-4 py-3 text-center text-[1.08rem] font-bold leading-tight text-white no-underline hover:bg-orange-dark sm:text-[1.15rem]">Accéder à la méthode</a>
    <p className={`mt-2 text-center text-[0.95rem] ${sombre ? "text-white/90" : "text-text-soft"}`}>{euros(montant)} · Paiement unique · Accès immédiat · Garantie 30 jours</p>
  </div>;
}
...
export default async function VslPage() {
  const jar = await cookies(), d = await devisFront(jar.get("hi_offre")?.value);
  return <>
    ...
    <main className="flex-1" data-version-vente="historique-v11">
      ...
      <div id="presentation-ecrite"><CalculHistorique /></div>
      <AvantApresHistorique />
      <div className="wrap"><section className="my-8 border-l-4 border-red bg-red-bg p-5">
    ...
    <ExitPopup storageKey="vsl-historique-v11" title="Ce que vous risquez si vous fermez cette page">
      <ul className="space-y-2 text-[1.03rem]">
        {[
          ...
          d.promotion.pourcent > 0 ? "Votre réduction personnelle ne se prolonge pas en quittant cette page. Le compteur ne repart pas de zéro au prochain passage." : "La question retournera dans la pile « plus tard ». Elle ne sera pas réglée pour autant.",
        ...
        ].map(t => <li key={t} className="flex gap-2 border-l-4 border-red bg-red-bg p-3"><span aria-hidden className="shrink-0 font-bold text-red">✕</span><span>{t}</span></li>)}
      </ul>
      <p className="text-[1rem] font-bold text-blue">Une première lecture, une première fiche. Commencez pendant que vous pouvez encore leur expliquer ce qui compte pour vous.</p>
      {d.promotion.pourcent > 0 && d.promotion.fin && <p className="text-sm text-text-soft">Votre palier de −{d.promotion.pourcent}% se termine le {new Date(d.promotion.fin).toLocaleString("fr-FR", { timeZone: "Europe/Paris", day: "numeric", month: "long", hour: "2-digit", minute: "2-digit" })}, heure de Paris. Le prix sera revérifié avant confirmation.</p>}
      <AccesMethode montant={d.montant} />
    </ExitPopup>
  </>;
}
