...
export async function livrer(order: Order): Promise<Acces | null> {
  try {
    ...
    //
    // Remise immédiate à Resend, délivrance différée de quelques heures : le
    // client ne reçoit pas deux messages collés après son achat.
    //
    // ⚠️ La première étape « c1 » recouvre largement l'email d'accès ci-dessus :
    // les deux disent « voici votre espace ». Le décalage les espace, il ne
    // supprime pas la redite — pour ça, c'est le contenu de c1 qu'il faut
    // réécrire, pas ce déclenchement.
    const premiere = SEQUENCE_CLIENT[0];
    if (r.ok && premiere) {
      const reserve = await reserverEnvoi(gagne.email, premiere.cle);
      if (reserve) {
        const c = await envoyerEtapeClient(reserve, premiere, momentPremiereEtape());
