...
for (const file of ["src/app/page.tsx", "src/app/methode/page.tsx"]) {
  ...
  ok(page.includes("<UrgencyBar />")); ok(page.includes("<UrgencyUnderButton />")); ok(page.includes("<ConditionsExoneration />")); ok(page.includes("votre mort")); ok(!page.includes("Pas une économie promise."));
  ok(page.includes("<ExempleHeadline")); ok(page.indexOf("<UrgencyBar") < page.indexOf("<Header"));
}
