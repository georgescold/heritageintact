...
for(const p of ["src/app/page.tsx","src/app/methode/page.tsx"]){const t=fs.readFileSync(p,"utf8");ok(t.includes("68 206 €"));ok(!t.includes("Écart de droits dans un exemple fictif à deux enfants, hors frais. Pas une économie promise."));ok(t.includes(p.includes("/methode/")?"votre mort":"7 erreurs à connaître à temps"));ok(t.includes("<UrgencyBar"));ok(t.includes("<ExitPopup"));ok(p.includes("/methode/")?t.includes("<CalculHistorique"):!t.includes("<ConditionsExoneration"));ok(!/Loys|Coquelle/.test(t));}
