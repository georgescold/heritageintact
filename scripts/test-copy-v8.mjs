...
for(const p of ["src/app/lp/page.tsx","src/app/methode/page.tsx"]){const t=fs.readFileSync(p,"utf8");ok(t.includes("68 206 €"));ok(!t.includes("Écart de droits dans un exemple fictif à deux enfants, hors frais. Pas une économie promise."));ok(t.includes(p.includes("/methode/")?"votre mort":"7 erreurs à absolument connaitre rapidement"));ok(t.includes("<UrgencyBar"));ok(t.includes("<SortieGuide"));ok(p.includes("/methode/")?t.includes("<CalculHistorique"):!t.includes("<ConditionsExoneration"));ok(!/Loys|Coquelle/.test(t));}
const lp=fs.readFileSync("src/app/lp/page.tsx","utf8"),optin=fs.readFileSync("src/components/OptinForm.tsx","utf8"),actions=fs.readFileSync("src/app/actions.ts","utf8");
ok(lp.includes('const cta = "Voir la vidéo"'));ok(lp.includes("7 erreurs à absolument connaitre rapidement"));ok(!lp.includes("L’exemple présenté sur la page suivante"));
