...
const espace=fs.readFileSync("src/app/espace/[jeton]/page.tsx","utf8");truth(espace.includes("Votre paiement a été accepté"));truth(espace.includes("Télécharger mon guide PDF"));truth(espace.includes("Votre Dossier Notaire est maintenant accessible"));truth(espace.includes("Votre plan adapté à votre situation est maintenant accessible"));truth(espace.includes("Votre guide Assurance-vie est maintenant accessible"));truth(!espace.includes("Votre complément est maintenant accessible"));
const guidesPdf=fs.readFileSync("src/lib/pdf-guides.ts","utf8");truth(guidesPdf.includes('slug:"lexique-succession"'));truth(guidesPdf.includes('categorie:"offert"'));const mesGuides=fs.readFileSync("src/components/espace/MesGuidesPdf.tsx","utf8");truth(mesGuides.includes(">Offert<"));truth(mesGuides.includes('g.categorie==="offert"'));
const boutique=fs.readFileSync("src/components/espace/Boutique.tsx","utf8");truth(boutique.includes("Construire mon plan adapté"));truth(boutique.includes("/plan-complet?o="));truth(boutique.includes("Indispensable pour les détenteurs d’une assurance-vie"));truth(boutique.includes("Découvrir le Dossier Notaire"));truth(boutique.includes('assurance ? "bg-green" : "bg-blue"'));truth(boutique.includes('assurance ? "border-green"'));
const config=fs.readFileSync("src/lib/config.ts","utf8");truth(config.includes("Mon plan adapté à ma situation"));truth(!config.includes("Mon simulateur + mon plan adapté"));
const offre=fs.readFileSync("src/components/OffrePreparation.tsx","utf8");truth(!offre.includes("profilComplet(profil)"));
const simulateur=fs.readFileSync("src/components/simulateur/SimulationPlan.tsx","utf8");truth(simulateur.includes("demarrerOffre({ objectif, vie: donnees.vie, enfants, age"));truth(simulateur.includes("QUESTION {index + 1} SUR {etapes.length}"));truth(!simulateur.includes("Simuler pour ma situation"));
for(const cle of ["age","vie","regime","residenceTotale","detentionResidence","souhaitResidence","quotePart","demembrement","immobilier","epargne","titres","autres","dettes","enfants","recomposition","beauxEnfants","descendantDecede","petitsEnfants","fratrie","neveux","sansLien","repartition","protectionSignee","personneConfiance","testament","avAvant","avApres","beneficiaires","donationAnnee","donationMontant","donationsMultiples","donationEpoux","international","entreprise","urgence","intention"])truth(simulateur.includes(`cle: "${cle}"`));
truth(!offre.includes("Simulez votre situation avant de laisser vos enfants découvrir les conséquences trop tard"));truth(offre.includes("L’État appliquera les règles aux faits et aux actes réellement en place"));
const comparatif=fs.readFileSync("src/components/ComparatifDecision.tsx","utf8");truth(comparatif.includes("ceux qui préparent leur rendez-vous chez le notaire"));truth(comparatif.includes("ceux qui vérifient leur assurance-vie"));truth(comparatif.includes("Le réflexe qui expose"));truth(comparatif.includes("Le réflexe qui protège"));truth(fs.readFileSync("src/app/espace/[jeton]/ajouter/[sku]/page.tsx","utf8").includes("<ComparatifDecision sku={sku}"));truth(offre.includes("<ComparatifDecision sku={sku}"));
