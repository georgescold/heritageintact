...
for(const gamme of ["front","suite"]){
 ...
 eq(palier(p,now).pourcent,50);
 const premiereFin=now+(gamme==="front"?5:10)*60000;
 eq(palier(p,premiereFin-1).pourcent,50);
 eq(palier(p,premiereFin).pourcent,gamme==="front"?0:34);
 const end=now+(gamme==="front"?5:15)*60000;
 eq(palier(p,end-1).pourcent,gamme==="front"?50:34);eq(palier(p,end).pourcent,0);eq(palier(p,end+1).fin,null);
 if(gamme==="suite"){eq(palier(p,now).montantFixe,147);eq(palier(p,premiereFin).montantFixe,197);}
...
eq(data.profils[0].email,"test@example.invalid");eq(data.promotions.length,1);
const startPlan=mod("src/app/profil.ts").demarrerOffrePlan;
eq((await startPlan(orderId,valide)).ok,true);eq(data.promotions.length,2);
const debutSuite=data.promotions.find(p=>p.gamme==="suite").commenceLe;
eq((await save(orderId,"",valide)).ok,true);
eq((await startPlan(orderId,valide)).ok,true);eq(data.promotions.find(p=>p.gamme==="suite").commenceLe,debutSuite);
const quote=mod("src/lib/devis.ts").devisPour;
eq((await quote("test@example.invalid","upsell1")).montant,147);
eq((await quote("test@example.invalid","upsell2")).montant,67);
eq((await quote("test@example.invalid","bump")).promotion.pourcent,0);
data.promotions.find(p=>p.gamme==="suite").commenceLe=new Date(Date.now()-11*60000).toISOString();
eq((await actions.chargeUpsell(orderId,"upsell1",147)).ok,false);
eq(data.orders[0].items.length,2);
eq((await quote("test@example.invalid","upsell1")).montant,197);
eq((await actions.chargeUpsell(orderId,"upsell1")).ok,false);
eq((await actions.chargeUpsell(orderId,"upsell1",197)).ok,true);
eq(data.orders[0].items.find(i=>i.sku==="upsell1").price,197);
const len=data.orders[0].items.length;
eq((await actions.chargeUpsell(orderId,"upsell1",197)).ok,true);eq(data.orders[0].items.length,len);
