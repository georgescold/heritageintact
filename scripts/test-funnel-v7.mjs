...
for(const gamme of ["front","suite"]){
 ...
 eq(palier(p,now).pourcent,gamme==="front"?20:25);
 eq(palier(p,now+1199999).pourcent,gamme==="front"?20:25);
 eq(palier(p,now+1200000).pourcent,10);
 const end=now+(gamme==="front"?7*86400000:48*3600000);
 eq(palier(p,end-1).pourcent,10);eq(palier(p,end).pourcent,0);eq(palier(p,end+1).fin,null);
...
for(const [v,p,r]of[[27,20,21.6],[27,10,24.3],[170,25,127.5],[153,25,114.75],[203,25,152.25],[0,25,0],[67,25,50.25]])eq(appliquerRemise(v,p),r);
...
let result=await actions.prepareCheckout({firstName:"Client",email:"test@example.invalid",consent:true,withBump:true,montantAffiche:27});
eq(result.ok,false);eq(result.actualiser,true);eq(data.orders.length,0);
result=await actions.prepareCheckout({firstName:"Client",email:"test@example.invalid",consent:true,withBump:true,montantAffiche:21.6});
eq(result.ok,true);const orderId=result.orderId;
eq(data.orders[0].items[0].price,21.6);eq(data.orders[0].items.reduce((s,i)=>s+i.price,0),38.6);
...
eq((await quote("test@example.invalid","upsell1")).montant,118.8);
eq((await quote("test@example.invalid","pack1")).montant,156.3);
...
eq((await quote("test@example.invalid","upsell1")).montant,142.56);
eq((await actions.chargeUpsell(orderId,"upsell1")).ok,false);
eq((await actions.chargeUpsell(orderId,"upsell1",142.56)).ok,true);
eq(data.orders[0].items.find(i=>i.sku==="upsell1").price,142.56);
const len=data.orders[0].items.length;
eq((await actions.chargeUpsell(orderId,"upsell1",142.56)).ok,true);eq(data.orders[0].items.length,len);
...
const mismatch=await actions.prepareCheckout({firstName:"Client",email:"autre@example.invalid",consent:true,withBump:false,montantAffiche:21.6});
