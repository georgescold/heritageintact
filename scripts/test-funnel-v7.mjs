...
eq((await startPlan(orderId)).ok,true);eq(data.promotions.length,2);
const debutSuite=data.promotions.find(p=>p.gamme==="suite").commenceLe;
eq((await save(orderId,"",valide)).ok,true);
eq((await startPlan(orderId)).ok,true);eq(data.promotions.find(p=>p.gamme==="suite").commenceLe,debutSuite);
